...
// AI Hub endpoints - Real-time AI data
router.get('/ai/models', async (req, res) => {
  try {
    const models = await liveDataService.fetchAITrendingModels();
    res.json({
      success: true,
      data: models,
      count: models.length,
...
router.get('/ai/datasets', async (req, res) => {
  try {
    const datasets = await liveDataService.fetchAIDatasets();
    res.json({
      success: true,
      data: datasets,
...
router.get('/ai/papers', async (req, res) => {
  try {
    const papers = await liveDataService.fetchAIPapers();
    res.json({
      success: true,
      data: papers,
      count: papers.length,
...
router.get('/ai/news', async (req, res) => {
  try {
    const news = await liveDataService.fetchAINews();
    res.json({
      success: true,
      data: news,
      count: news.length,
...
router.get('/ai/community', async (req, res) => {
  try {
    const posts = await liveDataService.fetchAICommunity();
    res.json({
      success: true,
      data: posts,
      count: posts.length,
...
router.get('/ai/repos', async (req, res) => {
  try {
    const repos = await liveDataService.fetchAIRepos();
    res.json({
      success: true,
      data: repos,
      count: repos.length,
...
router.get('/ai/dashboard', async (req, res) => {
  try {
    const dashboard = await liveDataService.fetchAIDashboard();
    res.json({
      success: true,
      data: dashboard,
...
// AI Hub aggregate endpoint
router.get('/ai/all', async (req, res) => {
  try {
    const [models, datasets, papers, news, community, repos] = await Promise.all([
      liveDataService.fetchAITrendingModels(),
      liveDataService.fetchAIDatasets(),
      liveDataService.fetchAIPapers(),
      liveDataService.fetchAINews(),
      liveDataService.fetchAICommunity(),
      liveDataService.fetchAIRepos()
    ]);
...
router.get('/live/weather', async (req, res) => {
  try {
    const weatherData = await liveDataService.fetchDjangoWeatherData();
...
router.get('/live/news', async (req, res) => {
  try {
    const newsData = await liveDataService.fetchDjangoNewsData();
...
router.get('/live/sports', async (req, res) => {
  try {
    const sportsData = await liveDataService.fetchDjangoSportsData();
...
router.get('/live/trending', async (req, res) => {
  try {
    const trendingData = await liveDataService.fetchDjangoTrendingData();
...
router.get('/live/courses', async (req, res) => {
  try {
    const coursesData = await liveDataService.fetchDjangoCoursesData();
...
router.get('/live/all', async (req, res) => {
  try {
    const [weather, news, sports, trending, courses] = await Promise.all([
      liveDataService.fetchDjangoWeatherData(),
      liveDataService.fetchDjangoNewsData(),
      liveDataService.fetchDjangoSportsData(),
      liveDataService.fetchDjangoTrendingData(),
      liveDataService.fetchDjangoCoursesData()
    ]);
...
function getCategoryDescription(category) {
  const descriptions = {
    ...
    // AI Hub categories
    ai_models: 'Trending AI models and machine learning frameworks from Hugging Face',
    ai_datasets: 'Popular AI datasets and training data from Hugging Face Hub',
    ai_papers: 'Latest AI research papers and publications from arXiv',
    ai_news: 'Breaking AI news and industry updates from multiple sources',
    ai_community: 'AI discussions and community posts from Hacker News and forums',
    ai_repos: 'Trending AI repositories and open-source projects from GitHub',
    ai_dashboard: 'Comprehensive AI hub dashboard with aggregated AI data and insights'
  };
