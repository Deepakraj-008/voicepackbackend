...
PERPLEXITY_KEY = _settings.PERPLEXITY_API_KEY or os.getenv("PERPLEXITY_API_KEY")
...
def _perplex_client() -> PerplexityClient | None:
    if not PERPLEXITY_KEY:
        return None
    try:
        return PerplexityClient(PERPLEXITY_KEY)
    except Exception:
        return None


@router.post("/chat")
async def ai_chat(prompt: str = Query(..., min_length=1)):
