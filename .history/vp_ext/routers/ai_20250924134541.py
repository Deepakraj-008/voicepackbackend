...
@router.post("/chat")
async def ai_chat(prompt: str = Query(..., min_length=1), db: Session = Depends(get_session), api_key: str = Depends(ai_auth)):
...
async def ws_chat_stream(websocket: WebSocket):
    ...
    """
    # Read api key from headers for WebSocket
    headers = {k.decode('latin1'): v.decode('latin1') for k,v in websocket.headers.raw}
    x_api_key = headers.get('x-api-key') or headers.get('X-API-KEY')
    try:
        # auth + rate-limit check
        await ai_auth(x_api_key)
